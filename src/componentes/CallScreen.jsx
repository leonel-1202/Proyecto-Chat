    ...
    <div style={{ position: "relative", width: 120, height: 120, display: "flex", alignItems: "center", justifyContent: "center" }}>
      {[1, 2, 3].map((i) => (
        <div key={i} style={{
          ...
          opacity: 0, animation: `pulseRing 2s ease-out ${i * 0.5}s infinite`,
        }} />
      ))}
...
function IncomingCall({ remoteUser, callType, onAccept, onReject }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.85)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, backdropFilter: "blur(8px)", animation: "fadeUp 0.2s ease-out both" }}>
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "1.6rem", fontWeight: 600, color: "white", marginBottom: 6 }}>{remoteUser.nombre}</div>
      ...
      <div style={{ display: "flex", gap: 48, marginTop: 16 }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <CallBtn onClick={onReject} color="#f43f5e" size={64}><Hang /></CallBtn>
          <span style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.5)" }}>Rechazar</span>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <CallBtn onClick={onAccept} color="#25d366" size={64}><Accept /></CallBtn>
...
function CallingBanner({ remoteUser, callType, onHangUp }) {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.88)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, backdropFilter: "blur(8px)" }}>
...
function AudioCall({ remoteUser, remoteAudioRef, isMuted, duration, onMute, onHangUp, attachRemoteStream }) {
  useEffect(() => {
    attachRemoteStream?.();
  }, []);

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "linear-gradient(160deg, var(--bg-sidebar) 0%, var(--bg-base) 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      <audio ref={remoteAudioRef} autoPlay playsInline style={{ display: "none" }} />

      <Avatar initials={remoteUser.initials} size="lg" />
      <div style={{ textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", fontWeight: 600, color: "var(--text-primary)", marginBottom: 6 }}>{remoteUser.nombre}</div>
        <div style={{ fontSize: "1rem", color: "var(--accent)", fontFamily: "monospace", letterSpacing: "0.1em" }}>{duration}</div>
      </div>
      <div style={{ display: "flex", gap: 24, marginTop: 32 }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <CallBtn onClick={onMute} color={isMuted ? "var(--accent)" : "rgba(255,255,255,0.12)"} size={56}>
...
function VideoCall({ remoteUser, localVideoRef, remoteVideoRef, isMuted, isCamOff, duration, onMute, onCam, onHangUp, attachRemoteStream }) {
  useEffect(() => {
    attachRemoteStream?.();
  }, []);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 500, background: "#000" }}>
      ...
      <video
        ...
        style={{ position: "absolute", bottom: 100, right: 16, width: 120, height: 160, objectFit: "cover", borderRadius: 12, border: "2px solid rgba(255,255,255,0.3)", background: "#111" }}
      />

      <div style={{ position: "absolute", top: 20, left: 0, right: 0, textAlign: "center" }}>
        <div style={{ fontFamily: "var(--font-display)", fontSize: "1.2rem", fontWeight: 600, color: "white", textShadow: "0 2px 8px rgba(0,0,0,0.6)" }}>{remoteUser.nombre}</div>
        <div style={{ fontSize: "0.9rem", color: "rgba(255,255,255,0.7)", fontFamily: "monospace", marginTop: 4 }}>{duration}</div>
      </div>
...
export default function CallScreen({
  ...
  attachRemoteStream,
  onAccept, onReject, onHangUp, onMute, onCam,
  CALL_STATE,
}) {
  const duration = formatDuration(callDuration);

  if (!remoteUser || callState === CALL_STATE.IDLE) return null;
  ...
  if (callState === CALL_STATE.ACTIVE && callType === "audio")
    return <AudioCall remoteUser={remoteUser} remoteAudioRef={remoteAudioRef} isMuted={isMuted} duration={duration} onMute={onMute} onHangUp={onHangUp} attachRemoteStream={attachRemoteStream} />;

  if (callState === CALL_STATE.ACTIVE && callType === "video")
    return <VideoCall remoteUser={remoteUser} localVideoRef={localVideoRef} remoteVideoRef={remoteVideoRef} isMuted={isMuted} isCamOff={isCamOff} duration={duration} onMute={onMute} onCam={onCam} onHangUp={onHangUp} attachRemoteStream={attachRemoteStream} />;
