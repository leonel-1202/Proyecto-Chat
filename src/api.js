...
export const clearChat     = (chatId) => api.delete(`/messages/clear/${encodeURIComponent(chatId)}`);

export const getConversaciones  = (phone) => api.get(`/conversations?phone=${encodeURIComponent(phone)}`);
export const crearConversacion  = (data)  => api.post('/conversations', data);
export const actualizarConv     = (chatId, data) => api.patch(`/conversations/${encodeURIComponent(chatId)}`, data);
